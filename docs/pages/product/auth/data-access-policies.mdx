...
_When a query [joins][ref-joins] cubes,_ the row-level filters of each cube apply to that
cube only. All joins Cube generates are `LEFT JOIN`s, and a policy on a joined cube does not
change that: rows of the other cubes are kept even when the joined cube has no row the current
user may see, with the members of the joined cube set to `NULL`.

<InfoBox>

This is what makes a policy differ from a filter in a query, which does turn a `LEFT JOIN`
into an `INNER JOIN`. Adding a policy to a cube is meant to restrict access to that cube, so
it must not change which rows of the other cubes in a query are returned.
